...
rm -rf /usr/src/*
rm -rf /usr/src/.*
git clone --branch releng/14.4 https://git.FreeBSD.org/src.git /usr/src
```
...
```sh
bastille bootstrap 14.4-RELEASE update
...
```sh
bastille list | awk '{print $2}' | while read -r jailname; do bastille stop $jailname; bastille upgrade $jailname 14.4-RELEASE; bastille start $jailname; done
```
Restart all jails
```sh
